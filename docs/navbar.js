 ...
 *
 * NOTE: This is the original vanilla-JS version kept for reference.
 * The Next.js project uses components/Navbar/index.tsx instead.
 */
(function () {
  // ── Inject CSS ─────────────────────────────────────────
  ...
  .nav-hamburger { display: none; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: 10px; background: transparent; border: none; cursor: pointer; color: #fff; transition: background 0.15s; padding: 0; flex-shrink: 0; }
  .nav-hamburger:hover { background: rgba(255,255,255,0.1); }
  .navbar.scrolled .nav-hamburger { color: #0f0f0f; }
  .navbar.scrolled .nav-hamburger:hover { background: rgba(0,0,0,0.06); }
  .nav-hamburger .bar { display: block; width: 18px; height: 1.5px; background: currentColor; border-radius: 2px; transition: transform 0.25s ease, opacity 0.2s ease; transform-origin: center; }
  ...
  .nav-mobile-menu { display: none; position: fixed; inset: 0; z-index: 101; background: #0a0a0a; flex-direction: column; }
  .nav-mobile-menu.open { display: flex; animation: mobileMenuIn 0.22s cubic-bezier(0.4,0,0.2,1) forwards; }
  .nav-mobile-topbar { display: flex; align-items: center; justify-content: space-between; padding: 18px 24px; flex-shrink: 0; }
  .nav-mobile-close { background: none; border: none; cursor: pointer; color: rgba(255,255,255,0.6); padding: 4px; display: flex; align-items: center; justify-content: center; transition: color 0.15s; }
  .nav-mobile-close:hover { color: #fff; }
  .nav-mobile-body { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding-bottom: 40px; }
  .nav-mobile-inner { display: flex; flex-direction: column; align-items: center; gap: 4px; }
  .nav-mobile-link { display: block; text-align: center; padding: 10px 32px; border-radius: 10px; font-size: 22px; font-weight: 300; letter-spacing: -0.2px; color: rgba(255,255,255,0.55); text-decoration: none; transition: color 0.15s, background 0.15s; }
  .nav-mobile-link:hover { color: #fff; background: rgba(255,255,255,0.05); }
  .nav-mobile-divider { display: block; height: 1px; background: rgba(255,255,255,0.08); width: calc(100% - 48px); margin: 16px 0; }
  .nav-mobile-actions { padding: 0 24px; display: flex; justify-content: center; gap: 10px; }
  .nav-mobile-actions .navbar-btn { height: 44px; padding: 0 24px; font-size: 14px; font-weight: 500; border-radius: 14px; }
  ...
  @media (max-width: 820px) { .nav-links { display: none; } .nav-actions { display: none; } .nav-hamburger { display: flex; } .navbar-inner { padding: 10px 20px; } .navbar.scrolled { padding: 8px 0; } }
`;
